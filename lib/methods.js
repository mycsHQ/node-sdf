...
function sdf(cmd, password = required('password'), options) {
  return new Promise(resolve => {
    if (!Object.values(cliCommands).includes(cmd)) throw new Error(`Command "${ cmd }" not available in sdf cli`);
...
function sdfCreateSuiteAppProject(
  ...
  return sdfCreateProject('2', { name, path, id, version, publisherId });
}
