...
const {
  dependencies: { javaClass, supplemental, maven }
} = require('../config');
...
const downloadFile = (url, filePath) =>
  new Promise((resolve, reject) => {
    ...
    request.on('response', response => {
      ...
      const splitPath = pathname.split(path.sep);

      let fileName = splitPath[splitPath.length - 1];
      if (headers['content-disposition']) {
        fileName = headers['content-disposition'].match(/filename=(.*)/)[1] || fileName;
      }

      const output = fs.createWriteStream(path.resolve(filePath, fileName));

      if (fileName.endsWith('.gz')) {
        response.pipe(zlib.createUnzip()).pipe(tar.extract(filePath));
      } else {
...
async function setup() {
  ...
  // check if installed as npm package
  const jrePath = path.resolve(...(fs.existsSync(nodeModules) ? [ nodeModules ] : [ '..', '..', 'node_modules' ]), 'node-jre', 'jre');
